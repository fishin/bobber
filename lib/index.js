...
exports.getCommits = function(path, compareCommit) {
    ...
    if (compareCommit) {
        command = command + ' ' + this.getLatestCommit(path) + '...' + compareCommit;
    }
...
exports.validateUrl = function(scm) {
    ...
    if (scm.url.match('http://github.com')) {
        // check to see if its valid
        var smelt = new Smelt({ dirPath: '/tmp' });
        var rewriteUrl = scm.url.replace('http://github.com','http://anon:anon@github.com');
