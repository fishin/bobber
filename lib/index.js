...
exports.checkoutCode = function (path, scm, pr) {
    ...
    var finishTime;
    //var simpleCommand = smelt.runCommandSync('git config push.default simple');
    //var travisHack1 = smelt.runCommandSync('git config user.name ficion');
    //var travisHack2 = smelt.runCommandSync('git config user.email ficion@ficion.net');
    var checkoutCommandResult = smelt.runCommandSync(checkoutCommand);
    ...
    if (status === 'succeeded') {
        ...
        if (pr) {
            ...
            var branchResult = smelt.runCommandSync(branchCommand);
            ...
            var mergeCommitResult = smelt.runCommandSync(mergeCommand);
...
exports.getLatestCommit = function (path) {
    ...
    var smelt = new Smelt({ dirPath: path });
    var result = smelt.runCommandSync('git rev-parse HEAD');
...
exports.getLatestRemoteCommit = function (scm) {
    ...
    var result = smelt.runCommandSync('git ls-remote --heads origin');
...
exports.validateUrl = function (scm) {
    ...
    if (scm.url.match('https://github.com')) {
        ...
        var result = smelt.runCommandSync('git ls-remote ' + rewriteUrl);
        if (result.stderr) {
            return false;
        }
        return true;
