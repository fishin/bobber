...
//    this.checkoutCodeSync = exports.checkoutCodeSync;
};
...
/*
exports.checkoutCodeSync = function (path, scm, pr) {
    ...
    var startTime = new Date().getTime();
    var finishTime;
    var checkoutCommandResult = smelt.runCommandSync({ command: checkoutCommand });
    commandResults.push(checkoutCommandResult);
    status = checkoutCommandResult.status;
    ...
    if (status === 'succeeded') {
        ...
        if (pr) {
            ...
            var branchResult = smelt.runCommandSync({ command: branchCommand });
            commandResults.push(branchResult);
            var mergeCommand = 'git pull ' + parsedUrl.proto + parsedUrl.delimiter + parsedUrl.repoUrl + '/' + pr.remoteUser + '/' + parsedUrl.repo + ' ' + scm.branch;
            var mergeCommitResult = smelt.runCommandSync({ command: mergeCommand });
            commandResults.push(mergeCommitResult);
            status = mergeCommitResult.status;
        }
    }
